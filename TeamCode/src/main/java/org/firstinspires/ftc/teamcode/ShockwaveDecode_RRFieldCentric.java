...
public class ShockwaveDecode_RRFieldCentric extends LinearOpMode {
    ...
    //private CRServo rbIntakeServo = null;
    ...
    private boolean rbPrev = false; // (kept; not used in auto trapdoor)
    ...
    private static final double RPM_FAR  = 3300;
    ...
    private static final double shooterF = 13.22;//13.84;//13.653 * 1.0431;
    ...
    private static final double F_BASE_NEAR = shooterF;  // your tuned F at V_BASE
    private static final double F_BASE_FAR   = 13.22; // starting point for 3200 (reduce ~4.5%)
    ...
    private static final double TARGET_DISTANCE_NEAR_CM = 150.0;
    private static final double TARGET_DISTANCE_FAR_CM = 290.0;
    ...
    // -------------------------------
    // AUTO TRAPDOOR / AUTO FLYWHEEL CONTROL (uses existing names)
    // -------------------------------
    private static final double RPM_READY_MARGIN = 100.0; // "vicinity" of targetRPM
    private static final double SHOT_DROP_RPM = 300.0;    // 350-400 rpm shot drop threshold
    private static final long   DOOR_MIN_CLOSE_MS = 230;  // anti-chatter: minimum close time after shot
    private static final long   TAG_VALID_GRACE_MS = 120; // anti-chatter: tag validity grace window

    private double lastActualRPM = 0.0;
    private long doorCloseUntilMs = 0;
    private long lastTagValidMs = 0;

    // Flywheel pre-spin when tag is visible (time-saving)
    private static final long FLYWHEEL_KEEPALIVE_MS = 1800; // keep spinning this long after tag loss

    // Gate stability / anti-dance
    private static final double RPM_OPEN_MARGIN_HIGH = 120.0; // open when within 120 RPM
    private static final double RPM_CLOSE_MARGIN_LOW = 260.0; // close if falls > 260 RPM below target (hysteresis)
    private static final long   DOOR_MIN_OPEN_MS = 260;       // once opened, keep open at least this long (prevents chatter)

    // Trackers
    private long lastTagSeenMs = 0;
    private long doorOpenSinceMs = 0;

    private boolean overrideHeld = false;
    private boolean r3Prev = false;

    // Gate stability (prevents flicker chatter)
    private static final long GATE_GRACE_MS = 250;   // ignore brief tag/tx/dist flicker for gate hold
    private long lastShootReadyMs = 0;               // last time shootReadyInstant was true
    ...
    public void runOpMode() {
        ...
        //rbIntakeServo = hardwareMap.get(CRServo.class, "rbintake"); // name in config
...
//                rbIntakeServo.setPower(1.0);    // run reverse while held
//
//            }
            ...
            // =====================
            // ADD THESE STATE VARS AT TOP OF CLASS (reuse names; add only what’s needed)
            // =====================
            // private boolean overrideHeld = false;     // (you currently declare overrideHeld as a local; move to class if you want)
            // private boolean r3Prev = false;           // edge detect for right stick button toggle

            // NOTE: your pasted block assigns F_BASE (but your earlier declaration made it final).
            // That part is outside this patch; keep whatever you actually have compiling.
            // This patch focuses on: (1) R3 toggle override, (2) sticky gate logic per your latest requirement.


            // ----------------
            // RPM MEASUREMENT (needed for auto trapdoor)
            // ----------------
            double actualTicksPerSec = shooterR.getVelocity();
            double actualRPM = (actualTicksPerSec / ENCODER_COUNTS_PER_REV) * 60.0;

            // ----------------
            // FAILSAFE OVERRIDE: TOGGLE on RIGHT STICK BUTTON (R3)
            // Press once -> manual override ON
            // Press again -> back to AUTO
            // ----------------
            boolean r3Now = gamepad1.right_stick_button || gamepad2.right_stick_button;
            if (r3Now && !r3Prev) {
                overrideHeld = !overrideHeld;
            }
            r3Prev = r3Now;

            // ----------------
            // TAG VALIDITY GRACE (anti-chatter)  (keep for flywheel pre-spin only)
            // ----------------
            long nowMs = (long) runtime.milliseconds();
            if (tag.valid) lastTagValidMs = nowMs;
            boolean tagValidStable = tag.valid || ((nowMs - lastTagValidMs) <= TAG_VALID_GRACE_MS);

            // ----------------
            // MANUAL TRAPDOOR (D-PAD) - ONLY IN OVERRIDE
            // ----------------
            if (overrideHeld) {
            ...
            // - Manual only in OVERRIDE
            // - Auto otherwise (tag pre-spin + shoot-intent gate)
            // ----------------
            if (overrideHeld) {
                ...
                // Keep your existing preset->TARGET_FLYWHEEL_RPM mapping
                TARGET_FLYWHEEL_RPM = (preset == FlywheelPreset.FAR) ? RPM_FAR : RPM_NEAR;
                // Set F_BASE to match the selected preset
                F_BASE = (preset == FlywheelPreset.FAR) ? F_BASE_FAR : F_BASE_NEAR;
                ...
                // ----------------
                // AUTO FLYWHEEL (pre-spin when tag seen) + AUTO TRAPDOOR (sticky open)
                // ----------------
                boolean autoAlign = autoAlignLB || autoAlignRB;

                // Choose preset based on which align is held (RB wins if both).
                if (autoAlignRB) preset = FlywheelPreset.FAR;
                else if (autoAlignLB) preset = FlywheelPreset.NEAR;

                TARGET_FLYWHEEL_RPM = (preset == FlywheelPreset.FAR) ? RPM_FAR : RPM_NEAR;
                // Set F_BASE to match the selected preset
                F_BASE = (preset == FlywheelPreset.FAR) ? F_BASE_FAR : F_BASE_NEAR;

                // Tag debounce for flywheel keepalive (not for gate)
                long nowMsAuto = (long) runtime.milliseconds();
                if (tag.valid) {
                    lastTagValidMs = nowMsAuto;
                    lastTagSeenMs = nowMsAuto;
                }
                boolean tagValidStableAuto = tag.valid || ((nowMsAuto - lastTagValidMs) <= TAG_VALID_GRACE_MS);

                boolean tagRecentlySeen = (nowMsAuto - lastTagSeenMs) <= FLYWHEEL_KEEPALIVE_MS;

                // ---------- FLYWHEEL CONTROL ----------
                // Spin flywheel whenever tag is visible (stable) OR was seen recently.
                boolean shouldSpinFlywheel = tagValidStableAuto || tagRecentlySeen;

                if (shouldSpinFlywheel) {
                    flywheelOn = true;
                    applyShooterPIDFWithVoltageComp(shooterR);
                    double targetTicksPerSec = (TARGET_FLYWHEEL_RPM / 60.0) * ENCODER_COUNTS_PER_REV;
                    shooterR.setVelocity(targetTicksPerSec);
                } else {
                    // Tag not seen -> flywheel OFF and gate CLOSED immediately
                    flywheelOn = false;
                    shooterR.setVelocity(0);
                    ...
                    doorCloseUntilMs = 0;
                    lastActualRPM = actualRPM;
                    // no continue needed; just fall through safely
                }

                // ---------- GATE CONDITIONS (IMMEDIATE tag.valid required for gate) ----------
                // "all other conditions met": driver intent + tag valid + distance + tx within deadband
                boolean distanceReady = false;
                boolean txReady = false;

                if (tag.valid) {
                    double distErr = tag.distanceCm - TARGET_DISTANCE_CM;
                    distanceReady = Math.abs(distErr) <= DIST_DEADBAND_CM;
                    txReady = Math.abs(tag.txDeg) <= TX_DEADBAND_DEG;
                }

                boolean shootReadyInstant = autoAlign && tag.valid && distanceReady && txReady;

                // Latch time when we are truly ready (used to hold gate stable through flicker)
                if (shootReadyInstant) {
                    lastShootReadyMs = nowMsAuto;
                }

                // STABLE readiness: once we’ve been ready, tolerate brief loss without slamming gate shut
                boolean shootReadyStable = shootReadyInstant || ((nowMsAuto - lastShootReadyMs) <= GATE_GRACE_MS);

                // ---------- TRAPDOOR AUTO (stable + sticky open) ----------
                if (!shootReadyStable) {
                    // Only close when we have been NOT-ready for more than the grace window
                    if (trapdoorOpen) {
                        trapdoorOpen = false;
                        trapdoor.setPosition(TRAPDOOR_CLOSED_POS);
                    }
                    ...
                    // We consider ourselves "ready" (stable). Door should stay open unless a shot is detected.

                    // Shot detection: close immediately on big RPM drop while gate is open
                    double rpmDrop = lastActualRPM - actualRPM;
                    boolean shotDetected = trapdoorOpen && (rpmDrop >= SHOT_DROP_RPM);

                    if (shotDetected) {
                        trapdoorOpen = false;
                        trapdoor.setPosition(TRAPDOOR_CLOSED_POS);
                        doorCloseUntilMs = nowMsAuto + DOOR_MIN_CLOSE_MS;
                    }

                    // If closed (initially or after shot), open when RPM has recovered and we are still stable-ready
                    if (!trapdoorOpen) {
                        boolean holdExpired = (nowMsAuto >= doorCloseUntilMs);
                        boolean rpmReady = (actualRPM >= (TARGET_FLYWHEEL_RPM - RPM_READY_MARGIN));

                        if (holdExpired && rpmReady && shootReadyInstant) {
                            // Use INSTANT readiness for opening (prevents opening during grace-only)
                            trapdoorOpen = true;
                            trapdoor.setPosition(TRAPDOOR_OPEN_POS);
                        }
                    }
                    // If open: do nothing. It stays open through brief tag flicker (because shootReadyStable stays true).
                }

            }
            // Update last RPM sample at end of this whole block (shot detection uses it)
            lastActualRPM = actualRPM;



            /// This code it starts ands stops the flywheel when LB or RB pressed and held which wastes time.
            /*else {
                // ----------------
                // AUTO FLYWHEEL + AUTO TRAPDOOR
                // Conditions:
                //  - autoAlignLB or autoAlignRB is held
                //  - tag valid (with grace)
                //  - distance within DIST_DEADBAND_CM
                //  - tx within TX_DEADBAND_DEG
                // Behavior:
                //  - flywheel ON only when the above are true
                //  - trapdoor opens when RPM is near target
                //  - after shot drop >= SHOT_DROP_RPM: close, then reopen when RPM returns near target
                //  - if tag invalid/null: flywheel OFF and trapdoor CLOSED
                // ----------------
                ...
                boolean shootReady = autoAlign && tagValidStable && distanceReady && txReady;

                if (!shootReady) {
                    // Tag invalid OR not aligned / not at distance -> safe state
                    flywheelOn = false;
                    shooterR.setVelocity(0);
                    ...
                    // Reset anti-chatter door hold window
                    doorCloseUntilMs = 0;

                } else {
                    // Shoot-ready: ensure flywheel is ON at correct preset
                    flywheelOn = true;
                    ...
                    // Shot detection (RPM drop) while door is open
                    double rpmDrop = lastActualRPM - actualRPM;
                    boolean shotDetected = trapdoorOpen && (rpmDrop >= SHOT_DROP_RPM);
                    ...
                    if (shotDetected) {
                        trapdoorOpen = false;
                        trapdoor.setPosition(TRAPDOOR_CLOSED_POS);
                        doorCloseUntilMs = nowMs + DOOR_MIN_CLOSE_MS;
                    }

                    // Re/open logic (no door dancing):
                    // - only open if door is currently closed,
                    // - close-hold window is over,
                    // - RPM is back in the vicinity of target
                    if (!trapdoorOpen) {
                        boolean holdExpired = (nowMs >= doorCloseUntilMs);
                        boolean rpmReady = (actualRPM >= (TARGET_FLYWHEEL_RPM - RPM_READY_MARGIN));
                        if (holdExpired && rpmReady) {
                            trapdoorOpen = true;
                            trapdoor.setPosition(TRAPDOOR_OPEN_POS);
                        }
                    }
            ...
            telemetry.addData("Override (R3)", overrideHeld ? "HELD" : "NO");
