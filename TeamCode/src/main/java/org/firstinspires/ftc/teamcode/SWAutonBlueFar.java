...
public class SWAutonBlueFar extends LinearOpMode {
    ...
    private static double TARGET_DISTANCE_CM_FAR = 295;  // FAR distance from your TeleOp
    private static double TARGET_DISTANCE_CM_NEAR = 150;  // NEAR distance from your TeleOp
...
//                utils.alignToTagTxAndDistanceCm(drive, 298, 2),
...
//        fShooter.setEnabled(false);
        ...
        // --- Start pose: C1, flywheel facing C6 (upfield = -X) ---
// If you already rely on Pinpoint seeding, keep your current startPose line.
// Otherwise explicitly seed:
        drive.localizer.setPose(new Pose2d(+60, -12, Math.toRadians(180))); // facing -X (toward row 6)

// ============================
// STEP 1: Vision align + hold 298 cm, then shoot preloads
// ============================
        fShooter.setEnabled(true);

// For BLUE goal alignment, make sure search omega is correct sign for your robot
        utils.setLimelightSearchOmega(0.25);
...
// ================================================================================
// PART 2: Go to A2, intake forward, pick up and go to C4 shootingPoseNear and shoot
// =================================================================================

        // Base pose AFTER vision align + shooting
        Pose2d basePose = drive.localizer.getPose();

// Desired relative move: to Row2 (1 tile upfield in your convention: X-)
// and to A2|B2 seam midpoint (1.5 tiles toward A: Y-)
        double xRow2 = basePose.position.x + 1.7 * tile;
        double ySeam = basePose.position.y + 1.20 * tile;

// SAFETY: never allow the target Y to go to the red side (+Y).
// This enforces that the robot always stays at or left of column C (Y <= current Y).
// If your basePose.y is already near 0, this prevents drifting into +Y.
        if (ySeam < basePose.position.y) {
            ySeam = basePose.position.y;
        }

// Face intake toward A during the final strafe into the seam
        double intakeHeadingTowardA2 = Math.toRadians(90.0);

// Build one action with two legs:
//  1) lineToX (keeps Y constant, avoids drifting into +Y/red)
//  2) strafeTo (move toward A to seam midpoint)
        Action goToA2Safe = drive.actionBuilder(basePose)
                .lineToX(xRow2)
                .turn(intakeHeadingTowardA2 - basePose.heading.toDouble()) // rotate to intake-toward-A
                .strafeTo(new Vector2d(xRow2, ySeam))
                .build();

        Action goToA2SafeWithIntake = utils.driveWithIntake(goToA2Safe, intake, 1.0);
        Actions.runBlocking(utils.withPoseTelemetry(goToA2SafeWithIntake, drive, nShooter, false));
...
// Step 1: pull off the A wall into a safe lane (reduces wall hugging before spline)
        double ySafeLane = poseAtA2.position.y - 1.0 * tile;   // move away from A wall toward C-ish
// If this moves the wrong direction on your robot, flip to + 1.0*tile.
        if (ySafeLane > poseAtA2.position.y) {
            ySafeLane = poseAtA2.position.y;
        }

// Keep same heading while we pull off the wall (don’t introduce extra rotation here)
        Pose2d poseSafeLane = new Pose2d(poseAtA2.position.x, ySafeLane, poseAtA2.heading.toDouble());

// Build a 2-stage return:
//  1) strafe off the wall to safe lane
//  2) spline to shootingPoseFar (now we’re away from boundaries)
        Action returnToFarSafeThenSpline = drive.actionBuilder(poseAtA2)
                .strafeTo(poseSafeLane.position)
                .build();

        Actions.runBlocking(utils.withPoseTelemetry(returnToFarSafeThenSpline, drive, nShooter, false));

// Now spline from the safe lane pose to shootingPoseFar
        Pose2d poseAfterSafeLane = drive.localizer.getPose();
        Action poseBackToShootingPoseFromA2 = utils.GoToPose(poseAfterSafeLane, drive, shootingPoseFar);

        utils.setLimelightSearchOmega(0.0);
        fShooter.setEnabled(true);

        Action poseBackToShootingPoseFromA2WithVision =
                utils.driveWithLimelightTx(poseBackToShootingPoseFromA2, drive);
...
// ================================================================================
// PART 3: From shootingPoseFar -> A3 pickup (intake forward) -> C4 -> aim A6 -> shoot
// ================================================================================

// Start from the far shooting pose (authoritative reference for this cycle)
        Pose2d basePose3 = shootingPoseFar;

// ----------------------------
// STEP 3A: Go to A3|B3 seam, intake forward, pick up
// ----------------------------

// Row1/C1-ish -> Row3 is +2 tiles in your working convention
        double xRow3 = basePose3.position.x + 2.70 * tile;

// C -> A3|B3 seam (use same seam magnitude you tuned on A2)
        double ySeam3 = basePose3.position.y + 1.20 * tile;

// Safety (your proven direction: +Y is the safe direction for these pickups)
//        if (ySeam3 < basePose3.position.y) {
//            ySeam3 = basePose3.position.y;
//        }

// Intake faces toward A for the seam strafe (your proven convention)
        double intakeHeadingTowardA3 = Math.toRadians(90.0);

// Foul-safe legs: move in X first (Y constant), then strafe in Y to seam
        Action goToA3Safe = drive.actionBuilder(basePose3)
                .lineToX(xRow3)
                .turn(intakeHeadingTowardA3 - basePose3.heading.toDouble())
                .strafeTo(new Vector2d(xRow3, ySeam3))
                .build();

        Action goToA3SafeWithIntake = utils.driveWithIntake(goToA3Safe, intake, 1.0);
        Actions.runBlocking(utils.withPoseTelemetry(goToA3SafeWithIntake, drive, nShooter, false));
...
//        // Back to shooting pose towards A6, then shoot the 3 pickup balls
//        Pose2d poseAtA3 = drive.localizer.getPose();
//        Action poseBackToShootingPoseFromA3 = utils.GoToPose(poseAtA3, drive, shootingPoseFar); //GoToPose(fromPose, drive, toPose) Go to from currentpose to a new topose
//        utils.setLimelightSearchOmega(0.0);
//        // Shooter ON only for the “with vision” return + align + shoot
//        fShooter.setEnabled(true);
...
//        // ============================================================
//        // PART 1 (BLUE): From C1, drive upfield and rotate shooter toward A6, shoot preloads
//        // ============================================================
//
//        Pose2d startPose = drive.localizer.getPose(); // robot placed at C1, shooter facing C6 (upfield)
//
//        double tile = FieldCoordinates.TILE;
//
//        // C1 -> C4 is 3 tiles upfield.
//        // In your “flywheel-forward = X-” start configuration, upfield is X-:
//        double targetX_C4 = startPose.position.x - 3.0 * tile;
//
//        // Aim offset from C4 to A6 is 45° left (2 up, 2 left)
////        double aimOffsetRad = Math.atan2(2.0, 2.0); // = 45 degrees
////
...
//        double rowsUpToRow5      = 1.6;   // C4 -> C5 lane (one row up)
//        double colsTowardB5      = 0.5;   // C -> B (one column toward A)
//        double colsTowardA5B5Mid  = 1.10;   // C -> seam between B and A
//
...
//        double rowsUpToRow4      = 0.6;   // C4 -> C4 lane (same lane)
//        double colsTowardB4      = 0.5;   // C -> B (one column toward A)
//        double colsTowardB4A4Mid  = 1.10;   // C -> seam between B and A
//
//        // Convert to inches with your Blue sign convention:
//        // upfield is X-, toward A is Y-
...
//        double colsTowardB3A3Mid  = 1.10;   // C -> seam between B and A
//
//        // Convert to inches with your Blue sign convention:
...
//        //   toward A (left)        = Y-
//        // After intaking artifacts go to StartPose and shoot from there
//        // ============================================================
//
//        // Shooter OFF during Intake + Travel
//        nShooter.setEnabled(false);
...
//        double rowsDownToRow2      = 0.4;   // C4 -> C2 lane (one row down)
//        double colsTowardB2      = 0.5;   // C -> B (one column toward A)
//        double colsTowardB2A2Mid  = 1.10;   // C -> seam between B and A
//
//        // Convert to inches with your Blue sign convention:
//        // downfield is X+, toward A is Y-
...
//        Action poseToStartPoseAsShootingPoseFromA2 = utils.GoToPose(poseAtA2, drive, shootingPoseFar);
//
//        // Blend Limelight yaw correction while driving back
//        Action poseToStartPoseAsShootingPoseFromA2WithVision =
//                utils.driveWithLimelightTx(poseToStartPoseAsShootingPoseFromA2, drive);
//
//        // Shooter ON only for the “with vision” return + align + shoot
//        fShooter.setEnabled(true);
//        utils.setLimelightSearchOmega(0.0);
//        Actions.runBlocking(utils.withPoseTelemetry(
//                poseToStartPoseAsShootingPoseFromA2WithVision, drive, nShooter, false
//        ));
//
//        utils.setLimelightSearchOmega(0.25);
//        Actions.runBlocking(utils.withPoseTelemetry(
