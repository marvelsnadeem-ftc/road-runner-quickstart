...
@Autonomous(name = "Shockwave Auton Red Near Goal", group = "Auto")
public class SWAutonRedNearGoal extends LinearOpMode {

    private final ElapsedTime runtime = new ElapsedTime();

    // Same timing constants as Blue Near
    ...
    public void runOpMode() throws InterruptedException {
        ...
        utils.setLimelightSearchOmega(-0.25); // -ve for Red, +ve for Blue // set this value only once.
        ...
        double yD4 = startPose.position.y + (-0.5 * FieldCoordinates.TILE);

//        double xD4 = startPose.position.x + (2 * FieldCoordinates.TILE);
//        double yD4 = startPose.position.y + (-2 * FieldCoordinates.TILE);
...
//                utils.withPoseTelemetry(moveToD4TurnAndShootPoseWithVision, drive, nShooter, false));
        ...
        Actions.runBlocking(
                utils.withPoseTelemetry(moveToD4WithVision, drive, nShooter, false));

        double tile = FieldCoordinates.TILE;

        // Example: move 2 tiles in +Y (field)
        //double targetX = startPose.position.y + 2.0 * tile;

//        Action driveStraightAlongFieldYMoveToD4 = drive.actionBuilder(startPose)
//                .lineToX(targetX)
//                .build();
//
//        Actions.runBlocking(utils.withPoseTelemetry(driveStraightAlongFieldYMoveToD4, drive, nShooter, false));

        // Vision search + lock (red pipeline selected beforehand)
        Actions.runBlocking(utils.withPoseTelemetry(
                utils.alignToTagTxAndDistanceCm(drive, TARGET_DISTANCE_CM, 1),
