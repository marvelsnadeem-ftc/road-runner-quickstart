...
public class SWAutonBlueNearGoal extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        Action burstA5      = buildIntakeBurst(intake, 3);
        Action burstA4      = buildIntakeBurst(intake, 3);
...
// Keep heading the same while translating to C4, then do the same +45° turn
        ...
        Actions.runBlocking(withPoseTelemetry(part1, drive, nShooter, false));
...
// Shoot 3 preloads (intake burst only; shooter already running)
        ...
        Action toA5B5SeamWithIntake = driveWithIntake(toA5B5SeamTurnEarly, intake, 1.0);
        Actions.runBlocking(withPoseTelemetry(toA5B5SeamWithIntake, drive, nShooter, false));

        // Back to shooting pose towards A6, then shoot the 3 pickup balls
        Pose2d poseAtA5 = drive.localizer.getPose();
        Action poseBackToShootingPoseFromA5 = GoToPose(poseAtA5, drive, shootingPoseNear); //GoToPose(fromPose, drive, toPose) Go to from currentpose to a new topose

        Actions.runBlocking(withPoseTelemetry(poseBackToShootingPoseFromA5, drive, nShooter, false));
        ...
        // Intake only while driving (stops exactly when the drive action ends)
        Action toA4B4SeamWithIntake = driveWithIntake(toA4B4SeamTurnEarly, intake, 1.0);
        Actions.runBlocking(withPoseTelemetry(toA4B4SeamWithIntake, drive, nShooter, false));

        // Back to shooting pose towards A6, then shoot the 3 pickup balls
        Pose2d poseAtA4 = drive.localizer.getPose();
        Action poseBackToShootingPoseFromA4 = GoToPose(poseAtA4, drive, shootingPoseNear); //GoToPose(fromPose, drive, toPose) Go to from currentpose to a new topose

        Actions.runBlocking(withPoseTelemetry(poseBackToShootingPoseFromA4, drive, nShooter, false));
        ...
        // Intake only while driving (stops exactly when the drive action ends)
        Action toA3B3SeamWithIntake = driveWithIntake(toA3B3SeamTurnEarly, intake, 1.0);
        Actions.runBlocking(withPoseTelemetry(toA3B3SeamWithIntake, drive, nShooter, false));

        // Back to shooting pose towards A6, then shoot the 3 pickup balls
        Pose2d poseAtA3 = drive.localizer.getPose();
        Action poseBackToShootingPoseFromA3 = GoToPose(poseAtA3, drive, shootingPoseNear); //GoToPose(fromPose, drive, toPose) Go to from currentpose to a new topose

        Actions.runBlocking(withPoseTelemetry(poseBackToShootingPoseFromA3, drive, nShooter, false));
        ...
        // Intake only while driving (stops exactly when the drive action ends)
        Action toA2B2SeamWithIntake = driveWithIntake(toA2B2SeamTurnEarly, intake, 1.0);
        Actions.runBlocking(withPoseTelemetry(toA2B2SeamWithIntake, drive, nShooter, false));

        // Back to shooting pose towards A6, then shoot the 3 pickup balls
        Pose2d poseAtA2 = drive.localizer.getPose();
        Pose2d shootingPoseFar = new Pose2d(110.7995, 25, Math.toRadians(25));
        Action poseBackToShootingPoseFromA2 = GoToPose(poseAtA2, drive, shootingPoseFar); //GoToPose(fromPose, drive, toPose) Go to from currentpose to a new topose

        Actions.runBlocking(withPoseTelemetry(poseBackToShootingPoseFromA2, drive, nShooter, false));
        Actions.runBlocking(withPoseTelemetry(
                new SequentialAction(
                        burstA2
                ),
    ...
    // Generic helper: go from one pose to another via a spline, using a reasonable tangent.
